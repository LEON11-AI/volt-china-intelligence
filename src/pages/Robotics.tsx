...
const engagementOptions = [
  {
    eyebrow: 'FREE BUYER TOOL',
    title: 'China Humanoid Robot Buyer Checklist 2026',
    description: 'A practical buyer-side checklist covering configuration, development access, software, simulation, support and procurement risks.',
    price: 'Free',
    cta: 'Open the Buyer Checklist',
    href: '/robotics/buyer-checklist',
    event: 'robotics_checklist_click' as const,
  },
  {
    eyebrow: 'BUYER-SIDE VERIFICATION REVIEW',
    title: 'Buyer-Side Verification Review',
    description: 'For teams that already have a use case, shortlisted platform, quotation or configuration that needs independent buyer-side review.',
    bullets: ['Configuration-specific evidence review', 'SDK, API, control-access, ROS and simulation workflow checks', 'Written manufacturer questions and responses', 'Support, warranty and delivery review', 'Unresolved risks and next-step recommendations'],
    price: 'From US$1,500 / project',
    cta: 'Request a Verification Review',
    href: '#robotics-requirement',
    event: 'verification_pricing_cta_click' as const,
    featured: true,
  },
  {
    eyebrow: 'PROCUREMENT COORDINATION',
    title: 'Procurement Coordination',
    description: 'For qualified projects moving from platform selection toward quotation, configuration confirmation and purchase coordination.',
    bullets: ['Multi-platform or multi-configuration comparison', 'Structured manufacturer communication', 'Quotation and configuration clarification', 'Delivery and support coordination', 'Unresolved commercial and technical issue tracking', 'Agreed procurement next steps'],
    price: 'From US$3,000 / project',
    cta: 'Tell Us About Your Project',
    href: '#robotics-requirement',
    event: 'procurement_pricing_cta_click' as const,
  },
];
...
const Robotics: React.FC = () => {
  ...
  useEffect(() => {
    const section = document.getElementById('robotics-engagement-options');
    if (!section || !('IntersectionObserver' in window)) return;
    const observer = new IntersectionObserver(([entry]) => {
      if (!entry.isIntersecting) return;
      trackRoboticsEvent('pricing_section_view');
      observer.disconnect();
    }, { threshold: 0.2 });
  ...
  <main>
    ...
    <section className="border-b border-slate-800 bg-slate-950 py-7 md:py-8"><div className="mx-auto flex max-w-6xl flex-col gap-5 px-4 sm:px-6 md:flex-row md:items-center md:justify-between lg:px-8"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-volt">Free Buyer Checklist</p><h2 className="mt-2 text-xl font-bold text-white md:text-2xl">China Humanoid Robot Buyer Checklist 2026</h2><p className="mt-2 max-w-2xl text-sm leading-relaxed text-slate-400">A printable, buyer-side tool with 24 review items, a pre-purchase gate, a requirements worksheet, and a supplier answer record.</p></div><a href="/robotics/buyer-checklist" onClick={() => trackRoboticsEvent('robotics_checklist_click')} className="inline-flex shrink-0 items-center justify-center gap-2 self-start rounded-lg border border-slate-700 bg-slate-900 px-5 py-3 text-sm font-bold text-white transition-colors hover:border-volt/70 hover:bg-slate-800 md:self-auto">Open the Checklist <i className="fa-solid fa-arrow-right text-xs" /></a></div></section>
    ...
    <section id="robotics-engagement-options" className="border-y border-slate-800 bg-slate-900 py-16 md:py-20"><div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8"><div className="mb-10 max-w-3xl"><p className="mb-4 text-sm font-semibold uppercase tracking-[0.2em] text-volt">Engagement Options</p><h2 className="text-3xl font-bold text-white md:text-4xl">Start with the level of support your project actually needs.</h2></div><div className="grid gap-5 lg:grid-cols-3">{engagementOptions.map((option) => <article key={option.eyebrow} className={`flex min-w-0 flex-col rounded-2xl border p-6 ${option.featured ? 'border-volt bg-slate-950 shadow-xl shadow-volt/10' : 'border-slate-800 bg-slate-950/70'}`}><p className="text-xs font-bold uppercase tracking-[0.16em] text-volt">{option.eyebrow}</p><h3 className="mt-4 text-2xl font-bold leading-tight text-white">{option.title}</h3><p className="mt-4 leading-relaxed text-slate-400">{option.description}</p>{option.bullets && <ul className="mt-6 space-y-3 text-sm leading-relaxed text-slate-300">{option.bullets.map((bullet) => <li key={bullet} className="flex gap-3"><i className="fa-solid fa-check mt-1 text-xs text-volt" aria-hidden="true" /><span>{bullet}</span></li>)}</ul>}<div className="mt-auto pt-8"><p className="text-xl font-bold text-white">{option.price}</p><a href={option.href} onClick={() => { if (option.event) trackRoboticsEvent(option.event); if (option.event === 'verification_pricing_cta_click') trackRoboticsEvent('request_verification'); }} className={`mt-5 inline-flex w-full items-center justify-center gap-2 rounded-lg px-5 py-3 text-sm font-bold transition-colors ${option.featured ? 'bg-volt text-white hover:bg-volt-hover' : 'border border-slate-700 bg-slate-900 text-white hover:border-volt/70 hover:bg-slate-800'}`}>{option.cta} <i className="fa-solid fa-arrow-right text-xs" aria-hidden="true" /></a></div></article>)}</div><div className="mt-6 rounded-xl border border-slate-800 bg-slate-950/70 p-5 text-sm leading-relaxed text-slate-400"><p>Final scope and pricing depend on project complexity, number of platforms, required manufacturer coordination and delivery requirements.</p><p className="mt-2">Hardware costs, freight, duties, taxes and third-party charges are not included unless explicitly stated in a proposal.</p></div></div></section>
